...
//Results route
router.get('/results/:searchType/:search/:page', withAuth, async (req, res) => {
    try {
        const playlistData = await Playlist.findAll({
            where: {
                user_id: req.session.user_id
        ...
        if (req.params.searchType == 'song') {
            ...
            if (result.results.artistmatches.artist.length != 0) {
                ...
                const pageNoRes = [{ name: 'No Result', artist: 'No Result' }];
                res.render('results', { pageNoRes, playlists, results: true, navbar: true });
            }
            ...
            if (result.results.albummatches.album.length != 0) {
